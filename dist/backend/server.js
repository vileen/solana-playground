...
async function createHolderSnapshot() {
    ...
    for (const asset of assets) {
        try {
            ...
            const type = asset.collectionType;
            const nftInfo = {
                mint: asset.id,
    ...
    const holders = Array.from(holderMap.entries()).map(([address, { count, gen1Count, infantCount, nfts }]) => ({
        address,
        nftCount: count,
        gen1Count,
        infantCount,
...
async function getTokenHolders(tokenAddress) {
    try {
        ...
        const tokenAccounts = await connection.getParsedTokenAccountsByOwner(new PublicKey(tokenAddress), { programId: TOKEN_PROGRAM_ID });
...
async function createTokenSnapshot() {
...
async function saveSocialProfile(walletAddress, socialData) {
...
app.get('/api/holders', async (req, res) => {
    try {
        ...
        const files = await readdir(DATA_DIR);
        let snapshots = await Promise.all(files
            .filter(f => f.startsWith('snapshot_'))
            .map(async (f) => {
            const content = await readFile(join(DATA_DIR, f), 'utf-8');
            return JSON.parse(content);
        ...
        if (search && typeof search === 'string') {
            ...
            holders = holders.filter((holder) => holder.address.toLowerCase().includes(searchLower));
        }
        res.json(holders);
    }
    catch (error) {
...
app.get('/api/social-profiles', async (req, res) => {
    try {
        ...
        const nftSnapshots = await Promise.all(files
            .filter(f => f.startsWith('snapshot_'))
            .map(async (f) => {
            const content = await readFile(join(DATA_DIR, f), 'utf-8');
            return JSON.parse(content);
